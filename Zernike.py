...
class Pupil:
    ...
    def __init__(self,
        ...
        pupil_px_size = 1/(px_size*img_size)
        self.pupil_pxs = NA/(wl*pupil_px_size)
        center = img_size/2
        self.center = center
        kx = 2*np.pi*(qx-center)/(px_size*img_size)
        ky = 2*np.pi*(qy-center)/(px_size*img_size)
        self.xslope,self.yslope = qx/(img_size)*2*np.pi/px_size ,  qy/(img_size)*2*np.pi/px_size
        #with np.errstate(sqrt ='ignore'):
        parabola0 = np.sqrt((2*np.pi*oil_n/wl)**2-kx**2-ky**2)
        parabola0[np.isnan(parabola0)] = 0
        self.parabola = parabola0*1

        w = img_size/2

        self.pupilmask = np.ones_like(qx)*(kx**2+ky**2<=(2*np.pi*NA/wl*k)**2)
...
class Zernike:
    ...
    def __init__(self,
        ...
        R1=lambda n,m,k,rho: (-1)**k * np.math.factorial(n-k)/(np.math.factorial(k) * np.math.factorial((n+m)/2-k) * np.math.factorial((n-m)/2-k)) * rho**(n-2*k)
    ...
    def genZernStack(self):
        out = np.array([self.Znm(*z) for z in self.zernInd] )
        self.zernStack = out
        return out
...
class FitZern:
    ...
    def fitOneZern(self,i,debug):
        ...
        if debug:
            ...
            print 'der1,2',der1,der2
            print 'likelihood',s2

        if  der2<>0:
            dw = der1/der2
            if debug: print 'dw*gamma',dw
        ...
        try:
            while epoch<nEpochs:
                ...
                if np.abs(dL)>stop:
                    ...
                    chi2 = chi2tmp
                    saveChi = chi2
                    print 'epoch ', self.epochs, 'chi2', chi2, 'LL',self.LL, 'dL', dL,'ll,cc_MSE',ll_MSE,cc_MSE # 'gammas ', self.gammas
                    count=0
                    ...
                    print 'Likelihood change is less than ',stop,', keep trying',count, 'chi2', chi2, 'LL',self.LL,'dL', dL,'ll,cc_MSE',ll_MSE,cc_MSE
                else:
...
class PhaseFitWrap(object):
    def __init__(self,stack=None,NA=1.49,zStep=.1,skip=0,n_oil=1.51,px_size=.11,wl = .64,zern_num = 36,smooth=0,fitPupil=True,fileName = None,plot = True):
        if fileName:
        ...
        self.pupil = Pupil(img_size=self.size,NA=self.NA,oil_n=self.n_oil,px_size=self.px_size,wl=self.wl)
...
class ZernPhaseLt(ZernPhase,object):
    def __init__(self, **kwargs):
        ...
        self.pupil = Pupil(img_size=img_size,NA=kwargs['NA'],px_size=kwargs['px'],oil_n=kwargs['n_oil'])
