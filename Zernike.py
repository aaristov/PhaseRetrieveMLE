...
class Pupil:
    ...
    def __init__(self,
        ...
        parabola0 = np.sqrt((2*np.pi*oil_n/wl)**2-(kx*self.pupilmask)**2-(ky*self.pupilmask)**2)
        #parabola0[np.isnan(parabola0)] = 0
        self.parabola = parabola0*par_mul
...
class PhaseFitWrap(object):
    def __init__(self,stack=None,NA=1.49,zStep=.1,skip=0,n_oil=1.51,px_size=.11,wl = .64,zern_num = 36,smooth=0,fitPupil=True,fileName = None,plot = False,par_mul = 1.):
        ...
        self.pupil = Pupil(img_size=self.size,NA=self.NA,oil_n=self.n_oil,px_size=self.px_size,wl=self.wl,par_mul=par_mul)
